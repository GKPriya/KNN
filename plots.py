...
def classify_and_plot_grid(cities, k=1):
    ...
    for longitude in range(-130, -60):
        ...
        for latitude in range(20, 55):
            prediction = knn.knn_classify(k, cities, [longitude, latitude])

            plots[prediction][0].append(longitude)

            plots[prediction][1].append(latitude)

    # create a scatter series for each language
    # See above plot_cities() to plot your prediction.
    # TODO

    for language, (lat, long) in plots.items():
        plt.scatter(lat, long, marker=markers[language], c=colors[language], label=language)

    plt.legend()

    plt.title(str(k) + " Nearest neighbours")
